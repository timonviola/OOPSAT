...
Pfc = psat_obj.Bus.Pg(a.bus);
Qfc = psat_obj.Bus.Qg(a.bus);

% Define reference active power
a.con(:,21) = Pfc;

% Fuel Cell stae variable initialization
psat_obj.DAE.x(a.Ik) = Sn.*Pfc./Vb./Vn;
Ik = psat_obj.DAE.x(a.Ik);
psat_obj.DAE.x(a.qH2) = 2*Kr.*Ik./Uopt;
qH2 = psat_obj.DAE.x(a.qH2);
psat_obj.DAE.x(a.pO2) = (qH2./rHO-Kr.*Ik)./KO2;
pO2 = psat_obj.DAE.x(a.pO2);
psat_obj.DAE.x(a.pH2) = (qH2 - 2*Kr.*Ik)./KH2;
pH2 = psat_obj.DAE.x(a.pH2);
psat_obj.DAE.x(a.pH2O) = 2*Kr.*Ik./KH2O;
pH2O = psat_obj.DAE.x(a.pH2O);
logarg = pH2.*sqrt(pO2)./pH2O;
psat_obj.DAE.x(a.Vk) = -r.*Ik./Vn + N0.*(E0+RTon2F.*log(logarg))./Vn;
...
psat_obj.DAE.x(a.m) = sqrt(((xt./psat_obj.DAE.y(a.vbus)./Vk./a.con(:,24)).^2) ...
                     .*(Pfc.^2+(Qfc+(Vb.^2)./xt).^2));

% Define reference AC voltage
a.con(:,22) = Vb+psat_obj.DAE.x(a.m)./Km;

% take into account SOFC status
psat_obj.DAE.x(a.Ik) = a.u.*Ik;
psat_obj.DAE.x(a.qH2) = a.u.*qH2;
psat_obj.DAE.x(a.pO2) = a.u.*pO2;
psat_obj.DAE.x(a.pH2) = a.u.*pH2;
psat_obj.DAE.x(a.pH2O) = a.u.*pH2O;
psat_obj.DAE.x(a.Vk) = a.u.*Vk;
psat_obj.DAE.x(a.m) = a.u.*psat_obj.DAE.x(a.m) + ~a.u;
