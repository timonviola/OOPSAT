function [Syneq,pf] = equiv(a,borderbus,gengroups,yi,psat_obj)
...
pf = cell(nborder,1);
for h = 1:nborder
  vj = psat_obj.DAE.y(borderbus(h)+psat_obj.Bus.n)*exp(sqrt(-1)*psat_obj.DAE.y(borderbus(h)));
  gen = gengroups{h};
  yij = yi{h};
  pfh = zeros(length(gen),1);
  for i = 1:length(gen)
    gbus = a.bus(gen(i));
    vi = psat_obj.DAE.y(gbus+psat_obj.Bus.n)*exp(sqrt(-1)*psat_obj.DAE.y(gbus));
...
Syneq(:,1) = borderbus+psat_obj.Bus.n; % bus index
Syneq(:,3) = getkv(psat_obj.Bus,borderbus,1); % voltage rating
for i = 1:nborder
  gen = gengroups{i};
...
for h = 1:nborder
  ...
  if psat_obj.Settings.conv
    Hs = pf{h}.*a.con(hh,18)*psat_obj.Settings.mva/mvaeq; % inertias
    Syneq(h,19) = sum(pf{h}.*a.con(hh,19)*psat_obj.Settings.mva/mvaeq); % damping
  else
  ...
  if psat_obj.Settings.conv
    Vb2old = getkv(psat_obj.Bus,a.bus(hh),2);
    k = mvas.*Vb2old./Vb2new/psat_obj.Settings.mva;
  else
    k = ones(length(hh),1);
  end
  for u = 1:length(udx)
