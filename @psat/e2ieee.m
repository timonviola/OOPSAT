function check = e2ieee(obj,varargin)
...
disp(['Opening PSAT file "',fullFilePath,'"...'])
...
if exist(fullFilePath,'file')
  ...
  disp(['File "',fullFilePath,'" not found or not an m-file'],2)
  check = 0;
  return
end
...
% ---------------------------------------------------------------
%% EVAL in previous section loaded the data
Bus = setup(Bus,obj);
Line = setup(Line,obj,Bus);
Twt = setup(Twt,obj,Bus,Line); %#ok
Shunt = setup(Shunt,obj,Bus);
PV = setup(PV,obj,Bus);
SW = setup(SW,obj,Bus,PV);
PQ = setup(PQ,obj,Bus);
PQgen = setup(PQgen,obj,Bus);
PQ = addgen(PQ,PQgen,obj,Bus);
...
newfile = [fullFilePath(1:end-2),'.cf'];
...
% Section Start card
card_l = b128;
card_l(1:19) = 'BRANCH DATA FOLLOWS';
