...
p.con(:,13)  = psat_obj.Settings.mva*Vb2old.*p.con(:,13)./p.con(:,3)./Vb2new;
p.con(:,14)  = psat_obj.Settings.mva*Vb2old.*p.con(:,14)./p.con(:,3)./Vb2new;
...
idx = find(p.con(:,16) == 2);
if ~isempty(idx)
  p.con(idx,12) = psat_obj.Settings.mva*p.con(idx,12)./p.con(idx,3);
end
