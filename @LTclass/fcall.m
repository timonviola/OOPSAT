...
if ty1
  error(ty1) = psat_obj.DAE.y(p.vr(ty1))-ref(ty1);
end
if ty2
...
psat_obj.DAE.f(p.mc) = p.u.*(k.*error-h.*mc);
%disp([md, mc, psat_obj.DAE.g(p.md), psat_obj.DAE.f(p.mc), psat_obj.DAE.g(p.v1), ...
%      psat_obj.DAE.g(p.v2), psat_obj.DAE.g(p.bus1), psat_obj.DAE.g(p.bus2)])

% non-windup limits
fm_windup(p.mc, mmax, mmin, 'pf',psat_obj)
