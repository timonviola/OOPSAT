...
fm_errv(p.con(:,3),'Svc',p.bus,psat_obj)
...
if p.ty1
  k = Vb2new(p.ty1).*p.con(p.ty1,2)./Vb2old(p.ty1)/psat_obj.Settings.mva;
  p.con(p.ty1,9) = k.*p.con(p.ty1,9);
  p.con(p.ty1,10) = k.*p.con(p.ty1,10);
end

if p.ty2
  k = psat_obj.Settings.mva*Vb2old(p.ty2)./p.con(p.ty2,2)./Vb2new(p.ty2);
