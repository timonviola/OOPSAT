...
a.vwa = psat_obj.DAE.x(a.vw);
psat_obj.DAE.y(a.ws) = psat_obj.DAE.x(a.vw);

% be sure that each wind turbine is associated with a single wind speed
wind_idx = [psat_obj.Cswt.wind.*psat_obj.Cswt.u; 
            psat_obj.Dfig.wind.*psat_obj.Dfig.u;
            psat_obj.Ddsg.wind.*psat_obj.Ddsg.u];
...
for i = 1:a.n
  t0 = psat_obj.Settings.t0;
  tf = psat_obj.Settings.tf;
  dt = a.con(i,5);
