...
switch a.type
 case 'area'
  ...
  area_buses = getarea(psat_obj.Bus,0,0);
 case 'region'
  str = 'Region ';
  msg = 'Region names does not match region number.';
  area_buses = getregion(psat_obj.Bus,0,0);
end
...
if ~isempty(a.con)
  ...
  sdx = find(a.con(:,2));
  if ~isempty(sdx)
    a.slack(sdx) = getint(psat_obj.Bus,a.con(sdx,2));
  end
...
for i = 1:a.n
  ...
  if ~isempty(area_i)
    bus_idx = getidx(psat_obj.Bus,area_i);
    a.bus{i} = getint(psat_obj.Bus,bus_idx)';
  end
end
