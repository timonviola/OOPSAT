function varargout = fjh2(p,type,psat_obj,varargin)
...
% JIJ -> Jacobian matrix (Line.n,psat_obj.Bus.n) from bus "i" to bus "j"
% HIJ -> Hessian matrix (psat_obj.Bus.n,psat_obj.Bus.n) from bus "i" to bus "j"
%
% FJI -> column vector (Line.n,1) of flows from bus "j" to bus "i"
% JJI -> Jacobian matrix (Line.n,psat_obj.Bus.n) from bus "j" to bus "i"
% HJI -> Hessian matrix (psat_obj.Bus.n,psat_obj.Bus.n) from bus "j" to bus "i"
%
...
tps = p.con(:,11).*exp(psat_obj.jay*p.con(:,12)*pi/180);
Vfr = psat_obj.DAE.y(p.vfr).*exp(psat_obj.jay*psat_obj.DAE.y(p.fr));
Vto = psat_obj.DAE.y(p.vto).*exp(psat_obj.jay*psat_obj.DAE.y(p.to));
r = p.con(:,8);
x = p.con(:,9);
chrg = p.u.*p.con(:,10)/2;
z = r + psat_obj.jay*x;
...
switch type
 case 1
  ...
  Vi = psat_obj.DAE.y(p.vfr);
  Vj = psat_obj.DAE.y(p.vto);
  ti = psat_obj.DAE.y(p.fr);
  tj = psat_obj.DAE.y(p.to);
  b0 = p.u.*p.con(:,10)/2;
  yij = abs(y);
  fij = angle(y);
  m = p.con(:,11);
