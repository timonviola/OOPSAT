function [borderbus,gengroups,yi,y0,zthvec] = equiv(a,fid,psat_obj)
% write equivalent transmission lines

buslist = psat_obj.EQUIV.buslist;
idx = [];
extbus = [];
borderbus = [];
yi = [];
...
while 1
  ...
  if length(idxi) == 1
    if isempty(find(buslist == idxi))
      disp(['psat_obj.Bus ',num2str(idxi),' is an isolated external bus.'])
...
Ybus = Ybus + ybus(psat_obj.Shunt,buslist);
Ybus = Ybus + ybus(psat_obj.PQ,buslist);

xbus = borderbus + psat_obj.Bus.n;

switch psat_obj.EQUIV.equivalent_method
  
 case 1 % Thevenin 
  
  % add generators internal impedance 
  Ybus = Ybus + ybus(psat_obj.Syn,buslist);
  ...
  if ~isempty(nozth)
    ...
    for i = 1:length(nozth);
      h = nozth(i);
      k = borderbus(h);
      disp(['Warning: psat_obj.Bus "',psat_obj.Bus.names{k},'" has zero Thevenin impedance.'])
      zth(h) = jay*1e-3;
    end
  end
 ...
 case 2 % REI
  ...
  for h = 1:nborder
    ...
    for hh = 1:nisland
      ...
      if ~isempty(jj)
        ...
        gdx = [];
        for u = 1:length(kk)
          sdx = findbus(psat_obj.Syn,kk(u));
          % sdx = [sdx; findbus(psat_obj.PV, kk(u))];
          % sdx = [sdx; findbus(psat_obj.SW, kk(u))];
          if ~isempty(sdx)
            udx = [udx; u];
            gdx = [gdx; sdx];
          end
  ...
  shdata(:,1) = xbus;
  shdata(:,2) = psat_obj.Settings.mva;
  shdata(:,3) = getkv(psat_obj.Bus,borderbus,1);
  shdata(:,4) = 50;
  shdata(:,5) = -real(y0);
  % shdata(:,6) = -imag(y0);
  shdata(:,7) = 1;
...
lndata(:,1) = borderbus;
lndata(:,2) = xbus;
lndata(:,3) = psat_obj.Settings.mva;
lndata(:,4) = getkv(psat_obj.Bus,borderbus,1);
lndata(:,5) = 50;
lndata(:,8) = real(zth);
lndata(:,9) = imag(zth);
lndata(:,16) = 1;
