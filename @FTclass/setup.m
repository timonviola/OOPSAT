...
if ~isempty(idx)
  psat_obj.fm_disp('Warning: The fault clearing time must be greater than the fault time',2);
  a.con(idx,6) = a.con(idx,6) + a.con(idx,5);
  psat_obj.fm_disp(fm_strjoin('         Fault #',int2str(idx), ...
                 ' at bus #',psat_obj.Bus.names(a.bus(idx)), ...
